...
Windows GitHub distribution uses unsigned per-user EXE installers and Portable ZIP archives. This route does not require MSIX or a certificate, but Windows may show Unknown publisher or SmartScreen warnings. macOS public distribution remains a credentialed signing and notarization operation.

## Windows EXE and Portable release
...
3. produces `CodexBridge-Windows-<arch>-<version>-Setup.exe` and `CodexBridge-Windows-<arch>-<version>-Portable.zip`;
...
The installer is fixed to `%LOCALAPPDATA%\Programs\CodexBridge`, requires no administrator privileges, creates a Start Menu shortcut, and uses a stable Inno Setup AppId for in-place upgrades. It does not delete `%LOCALAPPDATA%\CodexBridge\Service` during uninstall. The App routes background-start configuration through MSIX StartupTask only when package identity exists; the unpackaged build uses the current user's `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` value.

Unsigned EXE and ZIP assets must remain explicitly described as unsigned. Do not tell users to install a certificate or weaken system-wide trust. Users should verify `SHA256SUMS.txt` before choosing SmartScreen's **More info** → **Run anyway**. The MSIX workflow is retained as an optional manual path for a future trusted Publisher and signing pipeline.

### Optional MSIX path

`.github/workflows/windows-release.yml` is manual-only and separate from the EXE/Portable release. It exists for a trusted Publisher and signing environment; its App Installer feed and package-identity StartupTask semantics do not apply to the ordinary EXE or Portable assets. Do not imply that an EXE/Portable release automatically contains an MSIX, or that separately dispatched workflows share the same four-part build revision.
