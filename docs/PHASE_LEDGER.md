...
| Codex framing/initialize/models | phase 0 | implemented | isolated Swift process + LF JSON transport + dynamic catalog | `Prototypes/AppServerProbe`; versioned schema | build, 4 XCTest, self-test, real initialize and model/list | production dispatcher still in progress |
...
| Core/domain/persistence/security | phases 1/5 | blocked | implementation not started | plan | tests required | broad scope |
