...
| Local read-only MCP | phase 2 | implemented | strict per-session SDK servers behind a secret-path IPv4 loopback NIO listener; only five bounded read tools | `BridgeMCP`; `docs/MCP_SWIFT_SDK_INTEGRATION.md` | 26 tool/session/HTTP/in-memory and real pinned-SDK loopback XCTest | pinned Inspector pass, slow-reader pressure, Keychain composition and real Tunnel Host/Origin pending |
