...
public struct MCPServiceServerFactory: Sendable {
  ...
  package static func instructions(customInstructions: String) -> String {
    let base =
      ...
      + "A read-only value may always narrow the saved mode; selecting workspace-write requires "
      + "permission_mode_override=true. Network access follows native "
      ...
      + "with provider_id=deepseek-harness and a verified installation; it supports experimental fresh "
      ...
      + "reports the matching effective capabilities. Read-only may narrow the saved mode without "
      + "an override; workspace-write requires permission_mode_override=true and an explicit user request. "
      ...
      + "is injected by Bridge. Antigravity does not support Supervisor, interactive provider "
      + "approval, or network_access=true; a "
      + "provider permission denial is reported as task failure. "
