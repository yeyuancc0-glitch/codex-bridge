...
public struct MCPServiceServerFactory: Sendable {
  ...
  package static func instructions(customInstructions: String) -> String {
    let base =
      ...
      + "reports the matching effective capabilities. Use permission_mode_override=true only for "
      + "an explicit user request. Continue only with a provider_session_id returned by a terminal "
