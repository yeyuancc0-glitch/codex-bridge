...
public struct MCPServiceServerFactory: Sendable {
  ...
  package static func instructions(customInstructions: String) -> String {
    let base =
      ...
      + "read-only fresh sessions only, so omit thread_id, execution_model, execution_effort, "
      + "skill_name, supervisor_model and supervisor_effort, and use permission_mode=read-only. "
