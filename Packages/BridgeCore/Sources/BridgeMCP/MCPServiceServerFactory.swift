...
public struct MCPServiceServerFactory: Sendable {
  ...
  package static func instructions(customInstructions: String) -> String {
    let base =
      ...
      + "a value advertised for the selected model. For Antigravity, explicitly set provider_id "
      + "to antigravity. V1 is read-only only: omit permission_mode or explicitly request "
      + "read-only, never request workspace-write, and do not set network_access=true. Only "
      + "request model, effort, or continuation when list_agents reports the matching effective "
      + "capability. Continue only with a provider_session_id returned by a terminal Antigravity "
      + "task for the same project and installation. "
      ...
      + "check, never to fail or resubmit a task."
