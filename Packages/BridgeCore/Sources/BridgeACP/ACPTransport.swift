...
public struct ACPProcessTransportConfiguration: Sendable {
  ...
  public let inputEOFGracePeriod: Duration
  ...
  public init(
    ...
    inputEOFGracePeriod: Duration = .zero
  ) {
    self.argv = argv
    ...
    self.inputEOFGracePeriod = inputEOFGracePeriod
  }
}
...
public final class ACPProcessTransport: ACPTransport, @unchecked Sendable {
  ...
  private let inputEOFGracePeriod: Duration
  ...
  public static func launch(
    ...
    let transport = ACPProcessTransport(
      ...
      inputEOFGracePeriod: configuration.inputEOFGracePeriod,
  ...
  public func send(_ frame: Data) async throws {
    guard !frame.isEmpty, frame.count <= maximumFrameBytes, !frame.contains(0x0A) else {
      throw ACPError.oversizedFrame
    }
    let isClosing = lock.withLock { closing }
    guard !isClosing else { throw ACPError.transportClosed }
    var line = frame
    line.append(0x0A)
    do {
  ...
  public func close() async {
    ...
    if inputEOFGracePeriod > .zero, process.isRunning {
      let process = process
      let gracePeriod = inputEOFGracePeriod
      _ = await Task.detached {
        process.waitForExit(timeout: gracePeriod)
      }.value
    }
    if let task {
      task.cancel()
  ...
  private static func exitError(_ termination: ManagedProcessTermination) -> ACPError {
    switch termination {
