...
public actor TaskCoordinator {
  ...
  public func suspend(taskID: TaskID, reason: String? = nil) async throws -> TaskProjection {
    try await requestStop(taskID: taskID, outcome: .suspend, reason: reason)
  }
  ...
  public func complete(
    ...
    let completed = try await appendDomain(.completionRecorded, taskID: taskID)
  ...
  public func resolveRecovery(taskID: TaskID, to phase: TaskPhase) async throws -> TaskProjection {
    ...
    let projection = try await appendDomain(.recoveryResolved(to: phase), taskID: taskID)
    if phase == .preparing { scheduleStart(taskID) }
    if phase.isTerminal || phase == .suspended { try await releaseOwnedLocks(taskID) }
    return projection
  }
  ...
  private func startPreparedTask(_ taskID: TaskID) async {
    ...
    do {
      ...
      do {
        ...
        await consume(session.observations, taskID: taskID)
      } catch {
  ...
  private func consume(
    ...
    for await observation in observations {
      do {
        try await apply(observation, taskID: taskID)
      } catch {
        try? await recordRuntimeFailure(error, taskID: taskID)
        try? await releaseOwnedLocks(taskID)
        return
  ...
  private func apply(_ observation: TaskExecutionObservation, taskID: TaskID) async throws {
    ...
    case .codexApprovalRequested(let approvalID):
      ...
      _ = try await appendDomain(.codexApprovalRequested(approvalID), taskID: taskID)
    case .turnCompleted:
      _ = try await appendDomain(.turnCompleted, taskID: taskID)
    case .turnStopped:
      _ = try await appendDomain(.turnStopped, taskID: taskID)
      try await releaseOwnedLocks(taskID)
  ...
  private func recordRuntimeFailure(_ error: any Error, taskID: TaskID) async throws {
    let current = try await task(taskID)
  ...
  private func appendDomain(_ event: TaskEvent, taskID: TaskID) async throws -> TaskProjection {
    let current = try await task(taskID)
